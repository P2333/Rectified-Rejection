...
def attack_pgd(model, X, y, epsilon, alpha, attack_iters, restarts=1, norm='l_inf',
                threebranch=False, use_CWloss=False):
...
def get_args():
    ...
    parser.add_argument('--evalset', default='test', choices=['AutoAttack','test'])
    ...
    if args.evalset == 'test':
        ...
        for i, (X, y) in enumerate(test_batches):
            ...
            if args.target:
                ...
                delta = attack_pgd(model, X, y_target, epsilon, pgd_alpha, args.attack_iters, args.restarts, args.norm, target=True, threebranch=args.threebranch, use_CWloss=args.evalonCWloss)
            else:
                delta = attack_pgd(model, X, y, epsilon, pgd_alpha, args.attack_iters, args.restarts, args.norm, threebranch=args.threebranch, use_CWloss=args.evalonCWloss)
        ...
        print(acc,ro_acc,FPR_thred,ro_FPR_thred)
